...
class EtherFrame(object):
    ...
    sbox = (0xa3,0xd7,0x09,0x83,0xf8,0x48,0xf6,0xf4,0xb3,0x21,0x15,0x78,
    ...
    def checksum(self):
        # There's probably a more efficient way to do this.  Optimize later.
        new_hdr = struct.pack(frame_header_format, self.version,  self.type,
                              self.pdu_length, 0, self.pdu_number, self.flags)
        new_pkt = new_hdr + self.bytes[len(new_hdr):]
        sum, result = [0, 0, 0, 0], 0
        for i, b in enumerate(new_pkg):
            sum[i & 3] += sbox[b]
        for i in xrange(4):
